import { AnnounceEvent, AnnouncePeerSimultaneously } from '../../constants';
...
export const httpAnnounceSchema = {
    ...
    compact: (rawCompact: string): number => {
        ...
        return !Number.isNaN(compact) ? compact : 0;
    },

    no_peer_id: (rawNoPeerId: string): number => {
        const noPeerId = Number(rawNoPeerId);

        return !Number.isNaN(noPeerId) ? noPeerId : 0;
    },

    event: (rawEvent: AnnounceEvent): AnnounceEvent =>
        allowAnnounceEvents.includes(rawEvent) ? rawEvent : AnnounceEvent.UPDATE,

    numwant: (rawNumwant: string): number => {
        const numwant = Number(rawNumwant);

        return !Number.isNaN(numwant) ? numwant : AnnouncePeerSimultaneously.DEFAULT;
    },
};
