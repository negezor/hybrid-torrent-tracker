import { AnnounceEvent, AnnouncePeerSimultaneously, TrackerAction } from '../constants';
import { IncorrectRequestError } from '../errors';
import { intToIPv4, ipv4PeersToCompact, toUInt32, toUInt64 } from '../helpers';
import type {
...
export const parseConnectionRequest = (message: Buffer, headers: IUDPRequestHeaders): IConnectionRequestPayload => {
...
export const parseAnnounceRequest = (message: Buffer, headers: IUDPRequestHeaders): IAnnounceRequestPayload => {
    /* 32 bytes (80-84) */
...
export const parseScrapeRequest = (message: Buffer, headers: IUDPRequestHeaders): IScrapeRequestPayload => {
...
export const connectionResponse = ({ transactionId, connectionId }: IUDPConnectionResponse): Buffer =>
    Buffer.concat([
        CONNECTION_ACTION,
